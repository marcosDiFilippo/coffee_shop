...
public class ProductsPanel extends JPanel {
    ...
    public ProductsPanel() {
        ...
        String[] columnNames = {"Imagen", "ID", "Nombre", "Precio", "Estado", "Categoría", "Descripción", "CatID", "Acciones"};
        ...
        tableProducts.getColumnModel().getColumn(5).setPreferredWidth(150);
        tableProducts.getColumnModel().getColumn(8).setPreferredWidth(200);
        ...
        tableProducts.getColumnModel().getColumn(7).setWidth(0);
        ...
        tableProducts.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer() {
            private static final long serialVersionUID = 1L;
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                Component c = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
        ...
        tableProducts.getColumnModel().getColumn(8).setCellRenderer(new TableCellRenderer() {
        ...
        btnTableEdit.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (row != -1) {
                    String imgPath = (String) tableModel.getValueAt(row, 0);
                    Long id = (Long) tableModel.getValueAt(row, 1);
                    String name = (String) tableModel.getValueAt(row, 2);
                    BigDecimal price = (BigDecimal) tableModel.getValueAt(row, 3);
                    boolean active = tableModel.getValueAt(row, 4).equals("Disponible");
                    String desc = (String) tableModel.getValueAt(row, 6);
                    Long catId = (Long) tableModel.getValueAt(row, 7);
                    
                    TableCellEditor editor = tableProducts.getCellEditor();
                    if (editor != null) editor.stopCellEditing();
                    
                    ProductDTO dto = new ProductDTO(id, catId, name, desc, price, active, imgPath);
                    Dashboard dashboard = (Dashboard) SwingUtilities.getWindowAncestor(ProductsPanel.this);
        ...
        tableProducts.getColumnModel().getColumn(8).setCellEditor(new DefaultCellEditor(new JCheckBox()) {
            private static final long serialVersionUID = 1L;
            @Override
